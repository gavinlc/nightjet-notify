# NightJet Ticket Search

A React application that allows users to search for NightJet train connections and check ticket availability.

## Features

- Search for train connections between any two NightJet stations
- View available trains for a specific date
- See ticket prices for different comfort classes (Sleeping Car, Couchette, Seat)
...
```bash
git clone https://github.com/yourusername/nightjet-notify.git
cd nightjet-notify
```
...
1. Enter a departure station in the "From Station" field
2. Enter an arrival station in the "To Station" field
3. Select your desired travel date
4. Click "Search Trains" to see available connections
5. View ticket prices for different comfort classes

## API

This application uses the NightJet API to fetch train connections and ticket availability. The API endpoints used are:

- `/stations/find` - Search for stations
- `/connection/find/{from}/{to}/{date}/{time}` - Search for train connections
- `/destinations/offers/{train}/{from}/{to}/{timestamp}` - Get ticket prices
