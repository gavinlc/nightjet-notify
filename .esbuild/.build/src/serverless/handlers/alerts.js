...
// src/serverless/handlers/alerts.ts
var alerts_exports = {};
...
var getTrainOffers = async (trainName, from, to, timestamp) => {
  const response = await axios_default.get(`${API_BASE_URL}/destinations/offers/${trainName}/${from}/${to}/${timestamp}`);
  return response.data[0];
};
var api_default = axios_default.create({
...
var dynamoDB = import_lib_dynamodb.DynamoDBDocumentClient.from(dynamoDBClient);
...
var TABLE_NAME = process.env.ALERTS_TABLE || "nightjet-notify-alerts-dev";
var formatResponse = (statusCode, body) => {
  return {
    statusCode,
    headers: {
...
var getAlerts = async () => {
  try {
    const result = await dynamoDB.send(new import_lib_dynamodb.ScanCommand({
      TableName: TABLE_NAME
    }));
...
var deleteAlert = async (event) => {
  try {
    const id = event.pathParameters?.id;
    if (!id) {
      return formatResponse(400, { error: "Missing alert ID" });
    }
    await dynamoDB.send(new import_lib_dynamodb.DeleteCommand({
      TableName: TABLE_NAME,
      Key: { id }
    }));
  ...
  try {
    ...
    for (const alert of alertsToCheck) {
      try {
        ...
        await dynamoDB.send(new import_lib_dynamodb.UpdateCommand({
          TableName: TABLE_NAME,
          Key: { id: alert.id },
